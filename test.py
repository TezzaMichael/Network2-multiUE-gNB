    ...
    for ue in network.ue_list:
        ...
        for interface in ue.interfaces:
            if interface == "uesimtun0":
                ...
                print("-    upf_cld")
                print(ping_test(ue.name, interface,
                      network.upf_cld.interfaces["upf_cld-s3"]))
            elif interface == "uesimtun1":
                print(f"-    interface: {interface}")
                print("-    upf_mec")
                print(ping_test(ue.name, interface,
                      network.upf_mec.interfaces["upf_mec-s2"]))
...
def bandwidth(network):
    for upf in [network.upf_mec, network.upf_cld]:
        ...
        for ue in network.ue_list:
            ...
            command = ["docker", "exec", ue.name, "iperf3",
                       "-c", dest_ip, "-B", interface_ip, "-t", "5"]
...
def routing(network):
    ...
    for ue in network.ue_list:
        ...
        for interface in ue.interfaces:
            ...
            print(
                f"### Eseguo il ping {ue.name}[{interface}] -> {destination} ###")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        # Permette di formattare l'help in modo più leggibile
